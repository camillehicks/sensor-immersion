...
To start, use the GatorEnvironment ``||gatorenvironment:initialize||`` command start the sensor ``||basic:when the program starts||``
...
You are going to need to make a temporary program or use an old program (from Lesson 2) that has the ``||basic:ShowNumber||`` and gator environment ``||gatorenvironment:get||`` commands. 
...
We need to set a logic command to test if the measured CO2 is below your selected ’alarm value’. Use the gator environment ``||gatorenvironment:get||`` and ``||logic:0 < 0||``  commands to write this logic statement for when the observed CO2 is less than (<) your ‘alarm value’. 
...
Use the ``||basic:forever||``, ``||basic:Show Icon||``, ``||basic:Pause||`` and ``||logic:If True then||`` commands to determine what happens when the CO2 observed is below the alarm value. Adding a pause after you show the icon keeps it from going too fast. 
